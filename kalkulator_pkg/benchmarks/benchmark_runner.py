...
from .feynman_equations import FEYNMAN_EQUATIONS, FeynmanEquation, get_equations_by_complexity
...
def run_single_benchmark(
            ...
            if success:
                ...
                try:
                    ...
                    for i in range(len(X_test)):
                        subs = {symbols[v]: X_test[i, j] for j, v in enumerate(equation.variables)}
    ...
    exact_match = check_symbolic_equivalence(discovered_expr, equation.formula, equation.variables)
...
def run_feynman_benchmark(
    ...
    for noise in noise_levels:
        for i, eq in enumerate(equations):
            if verbose:
                print(f"[{i+1}/{len(equations)}] {eq.name}: {eq.description}...", end=" ")
