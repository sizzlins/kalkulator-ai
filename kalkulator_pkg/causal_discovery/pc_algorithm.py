...
class CausalGraph:
    ...
    def has_edge(self, node1: str, node2: str) -> bool:
        ...
        return ((node1, node2) in self.edges or 
                (node2, node1) in self.edges or
                (min(node1, node2), max(node1, node2)) in self.undirected)
...
def g2_test(
    ...
    def discretize(col):
        return np.digitize(col, np.percentile(col, np.linspace(0, 100, n_bins + 1)[1:-1]))
    ...
    if conditioning_set:
        ...
        for z_val in np.unique(z_combined):
            ...
            g2 = 2 * np.sum(observed[mask_pos] * np.log(observed[mask_pos] / expected[mask_pos]))
        ...
        mask_pos = (observed > 0) & (expected > 0)
        g2_total = 2 * np.sum(observed[mask_pos] * np.log(observed[mask_pos] / expected[mask_pos]))
        dof_total = (n_bins - 1) ** 2
