...
class SINDy:
    ...
    def _sequential_threshold(self, Theta: np.ndarray, dXdt: np.ndarray) -> np.ndarray:
            ...
            for j in range(n_vars):
                ...
                if np.sum(nonzero) > 0:
                    Xi[nonzero, j] = np.linalg.lstsq(
                        Theta_norm[:, nonzero], dXdt[:, j], rcond=None
                    )[0]
        ...
        if dXdt is None:
            ...
            for i in range(n_vars):
                dXdt[:, i] = estimate_derivative(
                    X[:, i], t, method=self.config.derivative_method, order=1
                )
        else:
    ...
    def score(self, X: np.ndarray, t: np.ndarray, metric: str = "r2") -> float:
        ...
        for i in range(X.shape[1]):
            dXdt_actual[:, i] = estimate_derivative(
                X[:, i], t, method=self.config.derivative_method
            )
