...
if __name__ == "__main__":
    from kalkulator_pkg.function_manager import (
        define_function, find_function_from_data, export_function_to_file
    )
    ...
    if success:
        print(f">>> E(2,4)=16, E(4,2)=8, E(10,1)=5, E(8,3)=36, find E(m,v)")
    ...
    if success:
        print(f">>> g(0)=1, g(1)=0.3679, g(2)=0.0183, g(3)=0.0001, find g(x)")
    ...
    if success:
        print(f">>> I(1)=100, I(2)=25, I(4)=6.25, I(5)=4, find I(d)")
    ...
    success, func_str, _, error = find_function_from_data(data, ["x"])
    if success:
        print(f">>> y(0)=1, y(1)=1.543, y(-1)=1.543, y(2)=3.762, find y(x)")
    ...
    if success:
        print(f">>> V(r,h) = 3.14159*r²*h")
        print(f">>> export Volume to demo_volume.py")
        print(f"{msg}")
        with open("demo_volume.py", 'r') as f:
