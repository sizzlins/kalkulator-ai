...
class TestUserReportedFailures(unittest.TestCase):
    def test_linear_plane(self):
        ...
        raw_data = [
            ([1, 1], 0), ([2, 0], 1), ([0, 2.5], 0), ([10, 5], 35), ([2, 2], 5),
            ([5, 1], 12), ([0, 0], -5), ([3, 3], 10), ([1, 4], 6),
            ([0.5, 0.5], -2.5), ([100, 1], 297), ([-1, -1], -10)
        ]
        X = np.array([d[0] for d in raw_data])
        y = np.array([float(d[1]) for d in raw_data])
        data_points = [(list(map(str, d[0])), d[1]) for d in raw_data]
        
        success, func_str, _, _ = solve_regression_stage(X, y, data_points, ['x', 'z'], include_transcendentals=True)
    ...
    def test_sqrt_interaction(self):
        ...
        raw_data = [
            ([1, 1], 1), ([4, 9], 6), ([2, 8], 4), ([100, 1], 10), ([0, 100], 0),
            ([3, 12], 6), ([16, 4], 8), ([25, 4], 10), ([5, 20], 10)
        ]
        X = np.array([d[0] for d in raw_data])
        y = np.array([float(d[1]) for d in raw_data])
        data_points = [(list(map(str, d[0])), d[1]) for d in raw_data]
        ...
        self.assertTrue("sqrt(x*y)" in func_str or "x^0.5*y^0.5" in func_str or "(x*y)^0.5" in func_str, msg=f"Got: {func_str}")
    ...
    def test_exponential_base_2(self):
        ...
        self.assertTrue("2^x" in func_str or "2.0^x" in func_str or "2.00^x" in func_str, msg=f"Got: {func_str}")
    ...
    def test_sigmoid(self):
        ...
        self.assertTrue("1/(1+exp(-x))" in func_str or "exp(x)/(1+exp(x))" in func_str, msg=f"Got: {func_str}")
    ...
    def test_softplus(self):
        ...
        self.assertTrue("log(1+exp(x))" in func_str or "ln(1+exp(x))" in func_str, msg=f"Got: {func_str}")
