...
class TestDampedMotion(unittest.TestCase):
    def test_boss_level_damped_sine(self):
        ...
        self.assertTrue("exp(-t)" in func_str or "exp(-1.0*t)" in func_str, f"Missing decay term: {func_str}")
        self.assertTrue("sin(t)" in func_str or "sin(1.0*t)" in func_str, f"Missing sine term: {func_str}")
