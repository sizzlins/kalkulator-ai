...
class TestRefinements(unittest.TestCase):
    def test_robust_regression_outlier(self):
        ...
        data_points = [(["1"], 2), (["2"], 4), (["3"], 100), (["4"], 8), (["5"], 10),
    ...
    def test_interaction_discovery(self):
        ...
        self.assertFalse("x^2" in func_str and "x^3" in func_str) # Should not be polynomial approx
    ...
    def test_sindy_small_data(self):
        ...
        dx_dt = eqs.get('dx/dt', '')
        self.assertTrue("-x" in dx_dt or "-1*x" in dx_dt or "-0.99" in dx_dt or "-0.9" in dx_dt)
