...
class TestPhysicsBenchmarks(unittest.TestCase):
    ...
    def test_high_frequency_sine(self):
        ...
        self.assertTrue("sin(10*t)" in func_str or "sin(11*t)" in func_str, msg=f"Got: {func_str}")
    ...
    def test_arrhenius(self):
        ...
        limit_data = [
            (300, 3.567), (400, 8.208), (500, 13.53), (1000, 36.78),
            (200, 0.673), (100, 0.003), (1500, 51.34), (2000, 60.65)
        ]
        ...
        self.assertTrue("exp(-1000/T)" in func_str or "exp(-500/T)" in func_str or "exp(-1044/T)" in func_str, msg=f"Got: {func_str}")

    def test_cone_area(self):
        # Case A(r, h): User data points
        print("\n--- Testing Cone Area A(r, h) ---")
        user_points = [
            (3, 4, 47.1238898), (1, 1, 4.4428829), (5, 12, 204.20352),
            (2, 2, 17.77153), (0.5, 1, 1.75620), (10, 20, 702.48147),
            (1, 0, 3.14159), (0, 5, 0.0), (2, 3, 22.65434)
        ]
        ...
        success, func_str, _, _ = solve_regression_stage(X, y, data_points, ['r', 'h'], include_transcendentals=True)
        print(f"Cone Area Result: {func_str}")
        # Expect pi*r*sqrt(r^2+h^2)
        # pi approx 3.14. r*sqrt(...)
        self.assertTrue("sqrt" in func_str and "r^2" in func_str and ("3.14" in func_str or "pi" in func_str), msg=f"Got: {func_str}")

    def test_cone_sparse(self):
        # Case f(x, y) from User Request (N=7, with one BAD point)
        print("\n--- Testing Cone Sparse f(x, y) with Outlier ---")
        user_f_points = [
            (0, 0, 0), (0, 1, 0), (1, 0, 3.14159265),
            (1, 1, 4.4428829), (1, 2, 7.0248147), (2, 1, 14.049629),
        ...
        success, func_str, _, _ = solve_regression_stage(X, y, data_points, ['x', 'y'], include_transcendentals=True)
        print(f"Sparse Cone Result: {func_str}")
        
        # Expect pi*x*sqrt(x^2+y^2)
        # Or at least x*sqrt interaction
        self.assertTrue("sqrt" in func_str and "x^2" in func_str, msg=f"Got: {func_str}")
