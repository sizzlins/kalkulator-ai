...
def test_triple_product():
    ...
    if result:
        ...
        if (("m*g*h" in result_clean or "m*h*g" in result_clean or 
             "g*m*h" in result_clean or "g*h*m" in result_clean or
             "h*m*g" in result_clean or "h*g*m" in result_clean) and 
            "^" not in result_clean):
...
def test_newtons_law():
    ...
    if result:
        result_clean = result.replace(" ", "")
        if ("m*a" in result_clean or "a*m" in result_clean) and "sin" not in result_clean and "exp" not in result_clean:
